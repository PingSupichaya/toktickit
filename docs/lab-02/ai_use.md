...
| 3 | In server/prisma add required field in database and add seeds follow  do not work out of scope. if you not sure please ask me | runs test to check table created correctly and seed is added. |
| 4 |for client, define CSS and Zen green theme and create the reusable components, create API to retrieve only active Requesters from the database, create Requester selection screen and implement context for remember requester and update header app with selected username. | รัน client กับ server เช็คว่าเอเจนต์ทำงานครบหรือไม่และมีส่วนไหนที่ต้องแก้เพื่อให้ตรงสเปค |
| 5 | ปรับ dropdown ของ select requester และแก้ App header ให้รองรับทั้ง desktop และ mobile| รัน client กับ server เพื่อดูว่า component ที่ให้ไปแก้มามีการเปลี่ยนแปลงหรือไม่และตรงกับ ui spec แค่ไหน |
