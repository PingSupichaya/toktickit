...
*(See reference mockup: "Login & Password Change" screen — email/password sign-in with validation, and a mandatory Change Password screen for initial-password users.)*
...
*(See reference mockup: Ticket Queue table with search bar, filters, sortable columns, and pagination.)*
...
*(See reference mockup: Ticket Detail screen with fields grouped at top, and tabs for Public Comments, Internal Notes, Attachments, and Service Actions below.)*
...
*(See reference mockup: Users list with search/filter, Create User button, and a side panel for creating/editing a user with Full Name, Email, Role, Active toggle, and initial-password controls.)*
